...
- **File Size Limit & Proactive Alert (1,000 Lines Threshold)**: Any source code file (whether `.jsx`, `.js`, or `.css`) exceeding 1,000 lines must trigger an immediate proactive alert to the user. You must proactively warn the user and provide a structured analysis and plan for code slimming, dead code elimination, and modular decomposition/splitting before continuing or appending code.
